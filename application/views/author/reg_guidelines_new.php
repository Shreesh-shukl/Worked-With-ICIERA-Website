<div id="container">
        <?php include("./application/views/includes/navigation.php"); ?>
        <section class="py-5">
            <div class="container">
                <div class="row">
                    <div class="col text-justify">
                        <div class="title-section">
                            <h1>Registration Fee</h1>
                        </div>
                        <table cellspacing="0" class="table table-striped">
                            <tbody>
                                <tr>
                                    <td>
                                    <p style="text-align:center"><strong>CATEGORY</strong></p>
                                    </td>
                                    <td colspan="2">
                                    <p style="text-align:center"><strong>INR</strong></p>
                                    </td>
                                    <td colspan="2">
                                    <p style="text-align:center"><strong>USD</strong></p>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                    <p style="text-align:center">&nbsp;</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">NON-IEEE member</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">IEEE member(After discount)</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">NON-IEEE member</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">IEEE member(After discount)</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                    <p style="text-align:center"><strong>AUTHOR</strong></p>
                                    </td>
                                    <td colspan="4">
                                    <p>&nbsp;</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                    <p>PROFESSIONALS/ FACULTY</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">7500</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">6500</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">US$&nbsp;&nbsp;200</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">US$&nbsp;&nbsp;170</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                    <p>FULL TIME STUDENT</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">7000</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">6000</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">US$&nbsp;&nbsp;170</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">US$&nbsp;&nbsp;150</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                    <p style="text-align:center">CO-AUTHOR&nbsp; (If One author has been registered &amp; other Co-Authors also wants to register)</p>
                                    </td>
                                    <td colspan="4">
                                    <p>&nbsp;</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                    <p>PROFESSIONALS/FACULTY</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">3000</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">2500</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">US$&nbsp;&nbsp;150</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">US$&nbsp;&nbsp;120</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                    <p>FULL TIME STUDENT</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">2500</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">2000</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">US$&nbsp;&nbsp;120</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">US$&nbsp;&nbsp;100</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                    <p style="text-align:center">NON-AUTHOR PARTICIPANTS</p>
                                    </td>
                                    <td colspan="4">
                                    <p style="text-align:center">&nbsp;</p>
                                    </td>
                                </tr>
                                <tr> 
                                    <td>
                                    <p>PROFESSIONALS (3 DAYS Registration)</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">4000</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">3500</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">-</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">-</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                    <p>PROFESSIONALS (1 DAY Registration)</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">3000</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">2500</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">-</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">-</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                    <p>FULL TIME STUDENT (3 DAYS Registration)</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">2500</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">2000</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">-</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">-</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                    <p>FULL TIME STUDENT (1 DAY Registration)</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">1200</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">900</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">-</p>
                                    </td>
                                    <td>
                                    <p style="text-align:center">-</p>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    <div class="col-lg-4 d-none">

                    </div>
                </div>
            </div>
        </section>